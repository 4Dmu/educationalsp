...
func main() {
	logger := getLogger("D:/Coding/Projects/educationalsp/log.txt")
...
func handleMessage(logger *log.Logger, writer io.Writer, state analysis.State, method string, contents []byte) {
	...
	case "initialize":
		...
		if err := json.Unmarshal(contents, &request); err != nil {
			logger.Printf("Hey, we could'nt parse this: %s", err)
		}
	...
	case "textDocument/codeAction":
		...
		reponse := state.TextDocumentCodeAction(request.ID, request.Params.TextDocument.URI)
	...
	case "textDocument/completion":
		...
		reponse := state.TextDocumentCompletion(request.ID, request.Params.TextDocument.URI)
